...
export default function MatchDetail({ match, onBack }: MatchDetailProps) {
  ...
  const loser = match.winnerId ? match.fighters.find(f => f.id !== match.winnerId) : null;
        ...
        <div className="p-6 border-b">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-gray-50 p-4 rounded">
              ...
              <div className="text-2xl font-bold text-gray-900">{formatCurrency(match.betUnitPrice)}</div>
            </div>
            <div className="bg-gray-50 p-4 rounded">
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b">
                  <th className="text-left py-2 px-4 font-medium text-gray-900">名前</th>
                  <th className="text-left py-2 px-4 font-medium text-gray-900">賭け口数</th>
                  <th className="text-left py-2 px-4 font-medium text-gray-900">賭け先</th>
                  <th className="text-left py-2 px-4 font-medium text-gray-900">賭け金額</th>
                  {results && <th className="text-left py-2 px-4 font-medium text-gray-900">配当</th>}
                </tr>
              </thead>
              <tbody>
                    ...
                    <tr key={bettor.id} className={`border-b ${isWinner ? 'bg-green-50' : ''}`}>
                      ...
                      <td className="py-3 px-4">
                        <span className={`px-2 py-1 rounded text-sm ${
                          isWinner ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-700'
                        }`}>
                          {fighter?.name}
                        </span>
                      </td>
                      <td className="py-3 px-4">{formatCurrency(bettor.betAmount * match.betUnitPrice)}</td>
                      {results && (
                        <td className="py-3 px-4">
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              <div className="bg-blue-50 p-4 rounded border border-blue-200">
                <div className="text-sm text-blue-600 font-medium">掛け金総額</div>
                <div className="text-2xl font-bold text-blue-900">{formatCurrency(results.totalBetAmount)}</div>
              </div>
              <div className="bg-green-50 p-4 rounded border border-green-200">
                <div className="text-sm text-green-600 font-medium">山分け総額</div>
                <div className="text-2xl font-bold text-green-900">{formatCurrency(results.totalPoolAmount)}</div>
              </div>
              <div className="bg-purple-50 p-4 rounded border border-purple-200">
                <div className="text-sm text-purple-600 font-medium">主催者利益</div>
                <div className="text-2xl font-bold text-purple-900">{formatCurrency(results.organizerProfit)}</div>
              </div>
              <div className="bg-gold-50 p-4 rounded border border-gold-200">
                <div className="text-sm text-gold-600 font-medium">選手賞金総額</div>
                <div className="text-2xl font-bold text-gold-900">{formatCurrency(results.fighterPrizeTotal)}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div className="bg-gold-50 p-4 rounded border border-gold-200">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm text-gold-600 font-medium">🏆 勝利選手賞金</div>
                    <div className="text-xl font-bold text-gold-900">{winner?.name}</div>
                  </div>
                  <div className="text-2xl font-bold text-gold-900">{formatCurrency(results.winnerPrize)}</div>
                </div>
              </div>
              <div className="bg-gray-50 p-4 rounded border border-gray-200">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm text-gray-600 font-medium">敗者選手賞金</div>
                    <div className="text-xl font-bold text-gray-700">{loser?.name}</div>
                  </div>
                  <div className="text-2xl font-bold text-gray-700">{formatCurrency(results.loserPrize)}</div>
                </div>
              </div>
            </div>
            ...
            {results.winningBettors.length > 0 && (
              <div>
                <h3 className="text-lg font-semibold mb-3">勝利賭け参加者の配当詳細</h3>
                <div className="bg-green-50 p-4 rounded border border-green-200">
